...
- 客户端推荐***Macdown***（开源Mou）和作业部落***Cmd Markdown***（或专业的Markdown Plus）。Cmd Markdown和StackEdit一样都提供了WYSIWYG快捷操作工具栏，支持轻量级和高级编辑模式（普通、Vim和Emacs）。
Cmd Markdown编辑、预览的渲染效果很漂亮，且预览生成内容目录。Cmd Markdown的缺点是编辑和预览窗口大小无法调整，不支持主题配置导致编辑语法高亮缺乏层次（例如H1~H6）。
